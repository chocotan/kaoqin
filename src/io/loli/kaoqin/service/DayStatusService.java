...
public class DayStatusService {
	...
	public void save(DayStatus d){
		...
		c.setTime(d.getDate());
		if(mss.findByYearAndMonth(c.get(Calendar.YEAR), c.get(Calendar.MONTH),d.getP().getId()).size()==0){
		...
		d.setM(mss.findByYearAndMonth(c.get(Calendar.YEAR), c.get(Calendar.MONTH),d.getP().getId()).get(0));
		dsd.save(d);
	}
