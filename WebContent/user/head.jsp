...
<script type="text/javascript">
	...
	function setHolidayColor() {
		$("#tr.holiday").css('background-color','red');
	}
	setHolidayColor();
	...
	function change(id, vid) {
		...
		$("td#dst-" + id).html("");
		var content = "<select id='dst-input-"+id+"'>";
		//生成6到20小时
		for ( var i = 6; i <= 20; i++) {
			if (i != starthour) {
				content += "<option>";
		...
		content += ":";
		//生成0到59分钟
		content += "<select id='dsm-input-"+id+"'>";
		...
		var content = "<select id='det-input-"+id+"'>";
		//生成0到23小时
		for ( var i = 0; i <= 23; i++) {
			if (i != endhour) {
				content += "<option>";
		...
		content += ":";
		//生成0到59分钟
		content += "<select id='dem-input-"+id+"'>";
		for ( var i = 0; i < 60; i+=10) {
			if (i != endmin) {
				content += "<option>";
			} else {
		...
		content = "<input type='button' onclick='update(" + id + "," + vid
				+ ")' id='dbt-button-" + id + "' value='"
				+ "<fmt:message key="form.done"/>" + "'>"
				+ "<input type='button' onclick='cancle(" + id + ")' value='"
	...
	function update(id, vid) {
		var starthour = $("#dst-input-" + id).val();
		var startmin = $("#dsm-input-" + id).val();
		var endhour = $("#det-input-" + id).val();
		var endmin = $("#dem-input-" + id).val();
		var worktime = $("#dwt-input-" + id).val();
		var breaktime = $("#dbt-input-" + id).val();
		var extratime = $("#dehh-input-" + id).val();
		var tip = $("#dtip-input-" + id).val();
		var action = "update";
	...
	function add(id) {
		...
		content = "<input type='text' size='1' id='dtip-input-"+id+"' value='"+tip+"'>";
		$("td#dtip-" + id).append(content);
		content = "<input type='button' onclick='addSubmit(" + id
				+ ")' id='dbt-button-" + id + "' value='"
								...
								$("td#dbt-" + id)
										.html(
												"<input type='button' onclick='change("
														...
														+ ","
														+ newid
														+ ")' value='<fmt:message key='change'></fmt:message>'>");
							}
						});
	}
</script>
